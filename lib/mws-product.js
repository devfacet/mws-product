...
exports = module.exports = function(options) {
  ...
  // Init vars
  var authOpt   = (options && options.auth)         ? options.auth        : null, // authentication
      mpOpt     = (options && options.marketplace)  ? options.marketplace : null, // marketplace
  ...
  mwsReqQryGen = function mwsReqQryGen(options) {
    ...
    if(query && typeof query == "object") for(key in query) returnData[key] = ('' + query[key]);
    ...
    if(authInfo.secretKey && method && host && path) {
      ...
      for(key in returnData) keys.push(key);
      keys = keys.sort();
      for(key in keys) qry[keys[key]] = returnData[keys[key]];
