...
exports = module.exports = function(iParam) {
  ...
  var iAuth     = (iParam && iParam.auth)         ? iParam.auth         : null, // authentication
      ...
      xmlParser = new mXML2JS.Parser({charkey: 'C$', attrkey: 'A$', explicitArray: false}),    // xml parser
  ...
  mwsReqSignGen = function mwsReqSignGen(iStr) {
    return (iStr && typeof iStr == 'string' && authInfo.secretKey) ? mCrypto.createHmac('sha256', authInfo.secretKey).update(iStr).digest('base64') : null;
  };
  ...
  mwsReqQryGen = function mwsReqQryGen(iParam) {
    ...
    if(authInfo.secretKey && iMethod && iHost && iPath) {
      ...
      var tStr = [iMethod, iHost, iPath, mQS.stringify(tQry)].join("\n");
  ...
  serviceStatus = function serviceStatus(iCallback) {
    ...
    mRequest(reqOpt, function (err, res, body) {
      ...
      if(!reqErr) {
        if(mwsRes && mwsRes.data && mwsRes.data.GetServiceStatusResponse && mwsRes.data.GetServiceStatusResponse.GetServiceStatusResult)
        {
          returnData            = {};
          returnData.requestId  = mwsRes.data.GetServiceStatusResponse.ResponseMetadata.RequestId;
          returnData.status     = mwsRes.data.GetServiceStatusResponse.GetServiceStatusResult;
        }
        else {
          returnData = {"requestId": null, "status": null};
        }
      }
      else {
        returnErr = {
  ...
  matchingProductForId = function matchingProductForId(iParam, iCallback) {
    ...
    mRequest(reqOpt, function (err, res, body) {
      ...
      if(!reqErr) {
        if(mwsRes && mwsRes.data && mwsRes.data.GetMatchingProductForIdResponse && mwsRes.data.GetMatchingProductForIdResponse.GetMatchingProductForIdResult) {
          returnData            = {};
          returnData.requestId  = mwsRes.data.GetMatchingProductForIdResponse.ResponseMetadata.RequestId;

          if(mwsRes.data.GetMatchingProductForIdResponse.GetMatchingProductForIdResult instanceof Array) {
            returnData.products = mwsRes.data.GetMatchingProductForIdResponse.GetMatchingProductForIdResult;
          }
          else {
            returnData.products = [mwsRes.data.GetMatchingProductForIdResponse.GetMatchingProductForIdResult];
          }
          ...
          returnData = {"requestId": null, "products": null};
        }
      }
      else {
        returnErr = {
